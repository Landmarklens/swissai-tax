...
class TaxCalculation(Base):
    """
    Complete tax calculation for a session
    Stores all calculation details and results
    """
    ...
    calculation_type = Column(String(20))  # preliminary, final
    ...
    cantonal_tax = Column(Numeric(12, 2))
    municipal_tax = Column(Numeric(12, 2))
    church_tax = Column(Numeric(12, 2))
    total_tax = Column(Numeric(12, 2))

    # Detailed calculation data
    calculation_details = Column(JSON)  # Full breakdown

    created_at = Column(DateTime(timezone=True), server_default=func.now())

    # Relationships
    session = relationship("InterviewSession", back_populates="tax_calculations")

    def __repr__(self):
        return f"<TaxCalculation(id={self.id}, session_id={self.session_id}, total={self.total_tax})>"

    @property
    def refund_or_payment(self):
        """
        Calculate refund (positive) or additional payment (negative)
        This would compare total_tax with taxes already paid
        """
        # This will be calculated based on session data
        return None
