...
class TestGenevaTaxCalculator(unittest.TestCase):
    """Test suite for Geneva canton tax calculator"""

    def setUp(self):
        """Set up test fixtures"""
        self.calculator = GenevaTaxCalculator('GE', 2024)

    def test_initialization(self):
        """Test Geneva calculator initialization"""
        self.assertEqual(self.calculator.canton, 'GE')
        self.assertEqual(self.calculator.tax_year, 2024)
    ...
    def test_zero_income(self):
        """Test Geneva calculation with zero income"""
        tax = self.calculator.calculate(Decimal('0'), 'single')
        self.assertEqual(tax, Decimal('0'))

    def test_low_income_tax_free(self):
        """Test low income in tax-free bracket"""
        # Below 10000 is tax-free for single
    ...
    def test_married_more_favorable(self):
        """Test that married status has more favorable rates"""
        tax_single = self.calculator.calculate(Decimal('50000'), 'single')
        tax_married = self.calculator.calculate(Decimal('50000'), 'married')
        self.assertLess(tax_married, tax_single)
    ...
    def test_married_tax_free_threshold(self):
        ...
        # Below 17000 is tax-free for married
        tax = self.calculator.calculate(Decimal('15000'), 'married')
        self.assertEqual(tax, Decimal('0'))

    def test_top_bracket_rate_single(self):
        """Test top bracket rate for single taxpayers"""
        marginal_rate = self.calculator.get_marginal_rate(Decimal('300000'), 'single')
        self.assertEqual(marginal_rate, Decimal('0.175'))

    def test_top_bracket_rate_married(self):
        """Test top bracket rate for married taxpayers"""
        marginal_rate = self.calculator.get_marginal_rate(Decimal('200000'), 'married')
        self.assertEqual(marginal_rate, Decimal('0.15'))
...
class TestBaselStadtTaxCalculator(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures"""
        self.calculator = BaselStadtTaxCalculator('BS', 2024)
    ...
    def test_top_bracket_rate_single(self):
        ...
        rate = self.calculator.get_marginal_rate(Decimal('150000'), 'single')
        self.assertEqual(rate, Decimal('0.13'))

    def test_top_bracket_rate_married(self):
        """Test top bracket marginal rate for married"""
        rate = self.calculator.get_marginal_rate(Decimal('150000'), 'married')
