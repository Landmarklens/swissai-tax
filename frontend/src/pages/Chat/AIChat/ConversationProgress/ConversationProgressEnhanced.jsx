...
const ConversationProgressEnhanced = ({
  progress = 0,
  ...
  if (mode === 'form') {
    // Simplified progress for form mode
    return (
      <Paper
        elevation={0}
        sx={{
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
          ...
          {!profileCompleted && estimatedTime > 0 && (
            <Chip
              icon={<TimerIcon />}
              label={`~${formatTime(estimatedTime)} left`}
              size="small"
...
ConversationProgressEnhanced.propTypes = {
...
ConversationProgressEnhanced.defaultProps = {
  progress: 0,
  profileCompleted: false,
  completedSteps: [],
  currentStep: null,
...
export default ConversationProgressEnhanced;