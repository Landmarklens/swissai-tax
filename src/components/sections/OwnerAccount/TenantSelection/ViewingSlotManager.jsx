...
const ViewingSlotManager = ({ propertyId, onComplete }) => {
  ...
  const handleCreateSlot = async () => {
    const slotData = {
      slot_datetime: `${slotForm.date}T${slotForm.time}:00Z`,
    ...
    await dispatch(createViewingSlots({ 
      propertyId, 
      slots: [slotData] 
    }));
  ...
  const handleBulkCreate = async () => {
    ...
    await dispatch(bulkCreateSlots({ propertyId, slots }));
